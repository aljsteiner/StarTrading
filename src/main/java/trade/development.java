...
8/22/2024 use the mapArrays displayed in the display tab while running to find which settings values can be used with nudges to improve the winning of the clan given the smart additional process.   I have tried to sharpen the process of finding the best (most winning) value for a setting, and apply the winning value as the best - game value of the setting, and putting that value into the nudge for that setting  I haven't isolated the appropriate settings yet, I hope to derive them using the map process to find a significant best value for winning clan as opposed to the values for the loosing clan
8/16/2024 I use the ars arrays assigned to a particular named value "aiWorth" for the last fyW value of worth in Assets.CashFlow.yearEnd(), the values for each econ each year are broken into a set of less than 77 sections of values. The keys are ascii characters of which one of 77 characters are assigned to the named value position in the key.  The AIlims arrays of values, represent the top value of a setion of a named value.  The index of that value is the smallest value in the AIlims array that is greater than the input value of a values such as "aiWorth".
the Assets instance contains refereces for the AI values that are saved in Assets.CashFlow.saveAI,  These values are uses by Assets.CashFlow.saveAIKey when called at the start of the year in Assets.CashFlow.saveAIKey called in Assets.CashFlow.startAIyear called in Assets.CashFlow.aStartCashFlow called in Assets.assetsInit() called by Econ called by StarTrader.doYear()called by StarTrader.runAYear().
There are 3 types of named values, 1) settings values, 2) saved results used as input to Assets.CashFlow year, 3) saved reslts showing how the year went.
The hope is to find a set of one or more values that have a better input into the Econs of winners.  The idea is to identify those input values so that we can nudge the settings in a way to make this econ become a winner this year.  Winners are chosen at the end of StarTrader.doYear after each Econ has completed a year cycle.  This means Assets.saveAI sets the values that are kept in Assets for the next year.  this means the Assets.CashFlow.saveAI() sees the game values of winning clan and winning clan value are prevValues only that were input to that years CashFlow.  All the other named values such as aiWorth are curent result lastValues and prevValues going ito Assets.CashFlow.saveAIKey().  The winner is identified by the prev value that was used to produce a winner.
