...
// There are two contexts
  ...
  static final public String storyText
          = "         StarTrader       Version 19.66\n"
            ...
            + "\n"
            + "There are settings for the game-master that allows changing the ways to win, and the overall financial settings for the game.  There are also settings for 5 clans (groups of planets and ships) for up to 5 players.  The clans are preset so that a player can make settings for 1 or more clans.  For clans without a player,  the robots play with the original settings.\n"
            + "\n"
            + "I have started to work with a little bit of AI.  This is a very limited AI limited to run on home computers.  The AI gathers the history of multiple games, trying to determine settings used by winning clans, and apply those settings to the one or 2 AI clans “nudging” a couple of settings hoping to increase their score, and thus increase the clan score.  In addition, each year the game nudges one setting for some of the planets or ships of each clan, keeping track of whether this nudge increases the score for that planet or ship more rapidly or more slowly.  The idea is to find a better setting for the AIclan.\n"
            + "\n"
            + "The game is set initially so that the winner must be the most generous player.  The catch is it is not individual generosity.  But the cumulative generosity by all of the clan planets and ships.  Of course the more and the bigger the economies are, the more the clan can be generous.    \n"
            + "\n"
